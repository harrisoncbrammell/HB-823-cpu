...
module control_unit(
    ...
    always_comb begin
        // defaults first to prevent inferred latches
        PC_Sel     = 1'b0;
        A_Sel      = 1'b0;
        B_Sel      = 1'b0;
        ALU_select = ADD_OP;
        ...
        case (opcode)
            ...
            JAL: begin
                PC_Sel = 1'b1;    // jump always taken
                A_Sel = 1'b1;
                B_Sel = 1'b1;
                RegWEn = 1'b1;    // save return address
                WB_Sel = 2'b10;   // write back PC+1
                imm_sel = 1'b0;   // JAL uses 8-bit immediate
                ALU_select = ADD_OP;
            end
            ...
            JALR: begin
                ...
                // rd <- PC + 1
                PC_Sel = 1'b1;
                A_Sel = 1'b0;     // use dataR1 = rs1
                B_Sel = 1'b0;     // use dataR2 = R0 (encoded rs2 = 0)
                RegWEn = 1'b1;
                WB_Sel = 2'b10;   // write back PC+1
                imm_sel = 1'b0;
                ALU_select = ADD_OP;
            ...
            HALT: begin
                // leave enables at 0 so state stops changing
            end
